...
async function main() {
  ...
  try {
    ...
    const [contentGenre] = await myConn.query("SELECT * FROM content_genres");
    ...
    // Clear Neon tables
    await pgClient.query("TRUNCATE users, contents, genres, content_genres, osts, reviews, polls, poll_options CASCADE");
