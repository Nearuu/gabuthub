...
export default function Admin() {
  ...
  const handleDeleteUser = async (targetUser) => {
    ...
    if (!window.confirm(`Apakah Anda yakin ingin MENGHAPUS PERMANEN akun @${targetUser.username}? Kuasa Admin akan menghapus akun ini.`)) {
      return;
    }
    ...
    // Save to persistent exclusion list
    try {
      const deletedIds = getDeletedUserIds();
      ...
      // Remove from registered_users_list
      const stored = localStorage.getItem("registered_users_list");
      if (stored) {
        const list = JSON.parse(stored);
    ...
    toast.success(`Akun @${targetUser.username} berhasil DIHAPUS PERMANEN.`);
  };
